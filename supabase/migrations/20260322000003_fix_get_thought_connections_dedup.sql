-- Fix: get_thought_connections returned duplicate rows when edges existed
-- in both directions (A→B and B→A from backfill). DISTINCT ON deduplicates
-- by connected_thought_id, keeping the highest-similarity edge.
