-- Fix: count remaining BEFORE early-exit so dry-run (p_limit=0) reports correctly.
...
BEGIN
  -- Count remaining first (so dry-run with p_limit=0 reports correctly)
  ...
  -- Recount remaining after inserts
  SELECT count(*) INTO remaining_count
  FROM thoughts t
  WHERE t.embedding IS NOT NULL
    AND NOT EXISTS (
